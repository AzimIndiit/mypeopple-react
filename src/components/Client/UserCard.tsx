...
const UserCard: FC<UserCardProps> = ({ id, image, name,status }) => {
  ...
  return (
    <div className="p-[10px] xl:p-[15px]  flex flex-col justify-between  rounded-[16px] bg-white shadow-md w-full  gap-[16px]  xl:w-[258px] xl:h-[329px] border border-gray-200">
      <div className="flex items-center gap-2  xl:h-[200px] w-full xl:w-[228px] rounded-[16px] bg-gray-100  relative">
        <img
          src={image}
          alt="user"
      ...
      <div className="flex  justify-between items-center gap-2 font-primary">
        ...
        <div className="text-[14px] font-light">{experienceFormatter(exp)}</div> */}
      </div>
      <div className="flex flex-col xl:flex-row gap-2  text-[12px] font-semibold justify-center">
        ...
        <Button
          variant="outline"
          className="h-[40px] xl:w-[114px] "
          onClick={() => {
            const formattedId = id.toString().replace(/\s+/g, "-");
            if (formattedId) {
              navigate(`/clients/users/${formattedId}`);
            }
          }}
        >
          {" "}
